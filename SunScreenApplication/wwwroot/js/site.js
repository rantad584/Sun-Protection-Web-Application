...
function MaxUviDay(a, e) {
    ...
    if (uvInfoElement) {
        let uvLevelText = a === "" ? "" : "<span>Suggestion:</span>";

        if (a !== "") {
            let message = "";
            const baseUrl = `<a class="btn btn-outline-dark" style="margin-bottom: 10px;" href="/Home/SunscreenUsage">Sunscreen Usage Guide »</a>`;
            const baseUrl2 = `<a class="btn btn-outline-dark" style="margin-bottom: 10px;" href="/Home/ClothingRecommendations">Clothing Recommendations »</a>`;

            if (a <= 2) {
                message = "UV 1-2: under UV 3 experts agree that little sun protection is needed if exposure is less than 1 hour.";
            } else if (a <= 4) {
                message = "UV 3-4: This is a mild UV level which necessitates sun protection if you are outside for more than 30 minutes.";
            } else if (a <= 6) {
                message = "UV 5-6: High UV index that necessitates careful planning to avoid skin damage if outside. Fair skinned people will burn 20 minutes of unprotected exposure.";
            } else if (a <= 8) {
                message = "UV 7-8: Very High UV levels. Take diligent care in the sun as adults will burn in 15 minutes. Children younger than 4 will burn quicker!";
            } else {
                message = "UV 9-11+ Extreme UV levels. Strongly consider staying indoors between 10 am - 4 pm. If outside take care not to expose skin for long periods of time.";
            }

            uvLevelText += `<p>${message}</p>${baseUrl} ${baseUrl2}`;
        }

        uvInfoElement.innerHTML = uvLevelText;
    }
